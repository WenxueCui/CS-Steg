...
from model import CSNet_Color_Gray

import torch.nn as nn
import torch
import torchvision.transforms as transforms

gray = transforms.Gray()   # transform color to grayscale image
...
parser.add_argument('--meas_rate_control', default=1.5, type=float, help='bitdepth for the SQ')
parser.add_argument('--blocksize', default=32, type=int, help='bitdepth for the SQ')

parser.add_argument('--loadEpoch', default=64, type=int, help='load epoch number')
parser.add_argument('--generatorWeights', type=str, default='', help="path to generator weights (to continue training)")
# parser.add_argument('--generatorWeights', type=str, default='pre_epochs_rate_control_0_SQ_Color_Gray_subrate_0.3_meas_rate_control_1.5/netG_epoch_1_70.pth')
...
netG = CSNet_Color_Gray(UPSCALE_FACTOR, opt.sub_rate)
...
for epoch in range(LOAD_EPOCH+1, NUM_EPOCHS + 1):
    ...
    for data, target in train_bar:
        ...
        if torch.cuda.is_available():
            cover = cover.cuda()
        if epoch < PRE_EPOCHS:  # the first PRE_EPOCHS is for SMG module training. 
        ...
        else:  # training the Steg module.
            # keep the sampling layer unchanged.
    ...
    save_dir = 'epochs_Gray' + '_subrate_' + str(opt.sub_rate)
